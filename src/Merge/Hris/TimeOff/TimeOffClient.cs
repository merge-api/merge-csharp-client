...
public partial class TimeOffClient
{
    ...
    /// </code></example>
    public async Task<Pager<Merge.Hris.TimeOff>> ListAsync(
        TimeOffListRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    ...
    /// </code></example>
    public async Task<Merge.Hris.TimeOff> RetrieveAsync(
        string id,
        ...
        return await _client
            .Options.ExceptionHandler.TryCatchAsync(async () =>
            {
                ...
                if (response.StatusCode is >= 200 and < 400)
                {
                    ...
                    try
                    {
                        return JsonUtils.Deserialize<Merge.Hris.TimeOff>(responseBody)!;
                    }
                    catch (JsonException e)
                    {
                        throw new MergeException("Failed to deserialize response", e);
