...
namespace Merge.FileStorage.Drives;

public partial class DrivesClient
{
    private RawClient _client;

    internal DrivesClient(RawClient client)
    {
        _client = client;
    }
        ...
        return await _client
            .Options.ExceptionHandler.TryCatchAsync(async () =>
            {
                ...
                if (response.StatusCode is >= 200 and < 400)
                {
                    ...
                    try
                    {
                        return JsonUtils.Deserialize<PaginatedDriveList>(responseBody)!;
                    }
        ...
        return await _client
            .Options.ExceptionHandler.TryCatchAsync(async () =>
            {
                ...
                if (response.StatusCode is >= 200 and < 400)
                {
                    ...
                    try
                    {
                        return JsonUtils.Deserialize<Drive>(responseBody)!;
                    }
                    catch (JsonException e)
                    {
                        throw new MergeException("Failed to deserialize response", e);
